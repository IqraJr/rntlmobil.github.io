...
@section("content")
<section class="bg-primary2 py-10 p-4 flex flex-col min-h-screen">
    <h1 class="text-2xl font-bold mb-4 text-center" style="color: #fdfac7; font-family: 'Poppins';">Daftar Mobil</h1>
    ...
    <div class="overflow-y-auto max-h-[90vh] p-4">
        <div class="grid md:grid-cols-3 gap-8 mx-auto">
            @foreach($cars as $car)
                @if($car->stock > 0)
                    <div class="p-7 rounded-lg shadow-md" style="background-color: #fdfac7;">
                        @if($car->image_path)
                            <img src="{{ asset($car->image_path) }}" alt="{{ $car->name }}" class="w-full h-48 object-cover mb-4">
                        @else
                            <div class="bg-gray-200 w-full h-48 flex flex-col items-center justify-center mb-3">
                        ...
                        <a href="{{ route('rentals.create', ['car_id' => $car->id]) }}" <!-- From Uiverse.io by mobinkakei --> 
                        <button class="cta">
